    ...
    >
      <div className="max-w-site mx-auto px-6 lg:px-12">
        ...
        <motion.div
          ...
          style={{ border: '1px solid rgba(221,216,208,0.15)' }}
          variants={fade}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: '-80px' }}
        ...
        >
          {[
            ...
            <div
              key={s.step}
              className="px-6 py-8"
              style={{ borderRight: '1px solid rgba(221,216,208,0.15)' }}
            >
              ...
              <p className="text-f-sm" style={{ color: 'rgba(245,243,239,0.6)' }}>{s.desc}</p>
            </div>
          ))}
        </motion.div>
        ...
        <motion.div
          ...
          className="max-w-2xl"
        >
          <p className="eyebrow mb-6" style={{ color: 'rgba(245,243,239,0.4)' }}>
            Payment calculator
          </p>

          {/* Unit selector */}
          <div className="mb-5">
            <label htmlFor="unit-select" className="text-f-xs font-medium block mb-2" style={{ color: 'rgba(245,243,239,0.6)' }}>
              Select unit
            </label>
            <select
              id="unit-select"
              ...
              style={{
                ...
                border: '1px solid rgba(221,216,208,0.2)',
                color: 'var(--stone)',
                borderRadius: '2px',
                outline: 'none',
              }}
            >
              <optgroup label="Commercial (1st floor)">
                {allUnits.slice(0, 10).map((u) => (
                  <option key={u.id} value={u.id} style={{ background: 'var(--ink)' }}>{u.label}</option>
                ))}
              </optgroup>
          ...
          {/* Toggle */}
          <div className="flex mb-8" style={{ border: '1px solid rgba(221,216,208,0.2)', borderRadius: '2px', overflow: 'hidden' }}>
            {(['installment', 'lumpsum'] as const).map((m) => (
              <button
                ...
                style={{
                  ...
                  color:      mode === m ? 'var(--ink)' : 'rgba(245,243,239,0.5)',
                  letterSpacing: '0.1em',
                }}
              >
            ...
            >
              {rows.map((row, i) => (
                <div
                  ...
                  style={{ borderBottom: '1px solid rgba(221,216,208,0.12)' }}
                >
                  <span className="text-f-sm" style={{ color: 'rgba(245,243,239,0.6)' }}>{row.label}</span>
                  <span
          ...
          <p className="text-f-xs mt-6 italic" style={{ color: 'rgba(245,243,239,0.35)' }}>
            {paymentPlan.note}
          </p>
        </motion.div>
      </div>
